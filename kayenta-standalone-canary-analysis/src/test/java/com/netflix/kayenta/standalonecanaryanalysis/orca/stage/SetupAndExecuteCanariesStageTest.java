...
public class SetupAndExecuteCanariesStageTest {
  ...
  @Test
  public void
      test_that_calculateLifetime_uses_supplied_lifetime_if_start_and_end_time_not_provided() {
    long lifetimeInMinutes = 5L;
    Instant now = Instant.now();
    Instant later = null;
    CanaryAnalysisExecutionRequest request =
        CanaryAnalysisExecutionRequest.builder().lifetimeDurationMins(5L).build();
  ...
  @Test
  public void
      test_that_calculateStartAndEndForJudgement_has_expected_start_and_end_when_nothing_is_set_in_the_scopes() {
    CanaryAnalysisExecutionRequest request =
    ...
    for (int i = 1; i < 6; i++) {
      SetupAndExecuteCanariesStage.ScopeTimeConfig conf =
          stage.calculateStartAndEndForJudgement(request, i, intervalDuration);
  ...
  @Test
  public void
      test_that_calculateStartAndEndForJudgement_has_expected_start_and_end_when_nothing_is_set_in_the_scopes_with_warmup() {
    CanaryAnalysisExecutionRequest request =
        CanaryAnalysisExecutionRequest.builder()
            .scopes(ImmutableList.of(CanaryAnalysisExecutionRequestScope.builder().build()))
            .beginAfterMins(4L)
  ...
  @Test
  public void
      test_that_calculateStartAndEndForJudgement_has_expected_start_and_end_when_start_and_end_are_supplied_in_the_scopes() {
  ...
  @Test
  public void
      test_that_calculateStartAndEndForJudgement_has_expected_start_and_end_when_start_and_end_are_supplied_in_the_scopes_with_warmup() {
    Instant definedStart = now.minus(5, ChronoUnit.MINUTES);
    Instant definedEnd = now.plus(20, ChronoUnit.MINUTES);
  ...
  @Test
  public void
      test_that_calculateStartAndEndForJudgement_has_expected_start_and_end_when_lookback_is_defined() {
    CanaryAnalysisExecutionRequest request =
        CanaryAnalysisExecutionRequest.builder()
            .lookbackMins(5L)
            .scopes(ImmutableList.of(CanaryAnalysisExecutionRequestScope.builder().build()))
