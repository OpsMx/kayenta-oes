  ...
  private ValidatableResponse doCanaryExec(String scope) throws IOException {
    ...
    CanaryConfig canaryConfig =
        objectMapper.readValue(
            getClass().getClassLoader().getResourceAsStream(canaryConfigJson), CanaryConfig.class);
    ...
    CanaryClassifierThresholdsConfig canaryClassifierThresholdsConfig =
        CanaryClassifierThresholdsConfig.builder().marginal(marginal).pass(pass).build();
    ...
    CanaryScope experiment =
        new CanaryScope().setScope(scope).setStart(metricsReportingStartTime).setEnd(end);

    CanaryScopePair canaryScopePair =
        CanaryScopePair.builder().controlScope(control).experimentScope(experiment).build();
    ...
    do {
      response =
          when().get(String.format(getUrl("/canary/" + canaryExecutionId))).then().statusCode(200);
